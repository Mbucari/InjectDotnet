		...
		*(JMP_ABS*)m_pHookJump = new JMP_ABS(hookFunction);
			...
			*(JMP_ABS*)m_pHookJump = new JMP_ABS(hookFunction, (uint)m_pHookJump);
			...
			if ((m_Trampoline = Trampoline.Create(originalFunc, m_pHookJump + sizeof(JMP_ABS))) != null)
			{
				...
				NativeMethods.VirtualProtect(originalFunc, (nint)sizeof(JMP_REL), MemoryProtection.ExecuteReadWrite, &oldProtect);
		...
		public virtual bool InstallHook()
		{
			...
			lock (this)
			{
				...
				else
				{
					...
					MemoryProtection oldProtect;
					if (!NativeMethods.VirtualProtect(OriginalFunction, (nint)sizeof(ulong), MemoryProtection.ExecuteReadWrite, &oldProtect))
						return false;
...
# endif
			...
			bool installAfterCreate = true)
		{
			...
			if (export.FunctionName is string expName && NativeLibrary.TryGetExport(hModule, export.FunctionName, out nint originalFunc))
		...
		public virtual void Dispose()
		{
			if (!IsDisposed)
			{
				if (HasTrampoline)
				{
					...
					{
						MemoryProtection oldProtect;
						NativeMethods.VirtualProtect(m_Trampoline.TargetAddress, (nint)sizeof(ulong), MemoryProtection.ExecuteReadWrite, &oldProtect);
						*(ulong*)m_Trampoline.TargetAddress = m_OriginalEpBytes;
						//Restore IAT's protection
						NativeMethods.VirtualProtect(m_Trampoline.TargetAddress, (nint)sizeof(ulong), oldProtect, &oldProtect);
					}
				}
				else
					RemoveHook();
